...
const LaunchRequestHandler = {
  ...
  async handle(handlerInput) {
    ...
    if (!appSettings.hasPreviousPlaybackSession) {
      message = 'Welcome to Youtube Player, say "play", followed by a song name'
...
const ResumeIntentHandler = {
  ...
  async handle(handlerInput) {
    ...
    if (appSettings.hasPreviousPlaybackSession) {
      ...
      const speakOutput = 'No content to resume, please start a new Youtube Player session'
      return handlerInput.responseBuilder
        .speak(ssmlClean(speakOutput))
        .withShouldEndSession(true)
        .getResponse()
...
const AudioPlayerEventHandler = {
  ...
  Settings.latestOffsetInMilliseconds = getOffsetInMilliseconds(handlerInput)
      appSettings.hasPreviousPlaybackSession = true
      break
...
async function axiosGetVideoIds(searchParam) {
  ...
  const params = {
    ...
    keyParamBase: '&key=',
    keyParam: 'AIzaSyB61dTqU6MuAfEdXP9FgMbgJ3qI8vfi22I' 
...
const controller = {
   ...
   */
  stop(handlerInput) {
    const speakOutput = 'Pausing youtube player'
    return handlerInput.responseBuilder
      .speak(ssmlClean(speakOutput))
      .withShouldEndSession(true)
